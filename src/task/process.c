...
static struct process* processes[RASOS_MAX_PROCESSES] = {};
...
static int process_map_elf(struct process* process) {
    ...
    for (int i = 0; i < header->e_phnum; i++) {
        ...
        int flags = PAGING_IS_PRESENT | PAGING_ACCESS_BY_ALL;
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot) {
    ...
    program_stack_ptr = kzalloc(RASOS_USER_PROGRAM_STACK_SIZE);
...
int process_get_free_slot() {
    for (int i = 0; i < RASOS_MAX_PROCESSES; i++) {
        if (processes[i] == 0)
            return i;
    }
